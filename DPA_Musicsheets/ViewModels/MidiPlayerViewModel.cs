    ...
    public class MidiPlayerViewModel : ViewModelBase
    {
        ...
        public MidiPlayerViewModel(FileHandler fileHandler)
        {
            ...
            _sequencer.PlayingCompleted += (playingSender, playingEvent) =>
            {
                _sequencer.Stop();
                _running = false;
                fileHandler.State = new MusicEditingState(fileHandler);
            };
        ...
        public RelayCommand PlayCommand => new RelayCommand(() =>
        {
            if (!_running)
            {
                _fileHandler.State = new MusicPlayingState(_fileHandler);
                _running = true;
                _sequencer.Continue();
                UpdateButtons();
            }
        ...
        {
            _fileHandler.State = new MusicEditingState(_fileHandler);
            _running = false;
            _sequencer.Stop();
